...
审批卡片只展示必要的脱敏上下文，例如目录别名、动作类型数量、文件变更数量和扩展名、权限读写数量、网络目标域名；不展示命令正文、diff、完整路径、搜索词、reason 原文、完整环境变量、令牌或敏感文件内容。按钮只回写 `accept`、`acceptForSession`、`decline` 或 `cancel`，超过 `FCA_APPROVAL_TIMEOUT_SECONDS` 默认 `decline`。
