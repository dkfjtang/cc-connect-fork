...
- `FCA_CARD_CHANNEL` 已支持 `im` / `cardkit` 配置；默认 `im`，显式 `cardkit` 时 message client 会优先尝试 CardKit transport，失败或不可用时回退普通 IM 卡片，并保留 card channel / card id / sequence 元数据流转。
