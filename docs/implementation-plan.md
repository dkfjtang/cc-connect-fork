...
- `runDev` 已注册 `SIGINT` / `SIGTERM` 停机钩子，收到退出信号后记录 `bridge.shutdown_requested`，依次停止 Codex app-server 和飞书 transport；成功时记录 `bridge.stopped`，失败时记录 `bridge.shutdown_failed`，且 app 停止失败仍会继续尝试停止飞书 transport。
