...
- 附件下载 adapter 已先定义脱敏请求/结果契约，并提供默认 disabled 实现；事件层只把 `attachmentKind`、消息/会话维度和脱敏 pending id 交给 adapter，不传附件 key、文件名或内容。当前默认 adapter 只返回 `disabled`，不调用飞书下载 API、不写入本地文件、不把附件提交给 Codex。
