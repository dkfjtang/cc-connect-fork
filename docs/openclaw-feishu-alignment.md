...
| P1 | 卡片交互审批 | 已完成 Codex approval server request 识别、等待审批卡片和默认安全拒绝；飞书按钮回调待实现 |
...
| P1 | 敏感操作确认卡片 | 已映射到 Codex approval server request，并保留脱敏摘要；不复用 OpenClaw tool approval 内核，按钮回写后续实现 |
