...
| 群聊 @ 入口 | 群聊需要按 mention、群配置和发送者策略触发 | 已支持明确 @ 当前 bot 的群聊文本，并可用 `FCA_ALLOWED_GROUP_CHAT_IDS` 限制群 `chat_id`；仍以发送者 `open_id` 白名单为权限依据 |
...
| P1 | 群聊策略 | 已完成群聊 @ 最小入口和可选群 `chat_id` allowlist；群配置和 sender 细分策略后续补齐 |
